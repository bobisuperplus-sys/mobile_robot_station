...
def main():
    ...
    print(" [上位机接收测试命令 (黄金低延迟零绿屏管道)]:")
    print('   # 1. 接收车载前视相机 (front_cam, 5002 端口):')
    print('   gst-launch-1.0 udpsrc port=5002 buffer-size=2097152 caps="application/x-rtp,media=video,clock-rate=90000,encoding-name=H264,payload=96" ! rtpjitterbuffer latency=10 drop-on-latency=true ! rtph264depay ! h264parse ! avdec_h264 max-threads=2 ! videoconvert ! autovideosink sync=false')
    print('   # 2. 接收全局高空监控 (overview_cam, 5004 端口):')
    print('   gst-launch-1.0 udpsrc port=5004 buffer-size=2097152 caps="application/x-rtp,media=video,clock-rate=90000,encoding-name=H264,payload=96" ! rtpjitterbuffer latency=10 drop-on-latency=true ! rtph264depay ! h264parse ! avdec_h264 max-threads=2 ! videoconvert ! autovideosink sync=false')
    print(" [控制说明]:")
